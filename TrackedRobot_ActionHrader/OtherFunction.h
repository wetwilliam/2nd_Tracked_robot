...
void action_5f();	//s5
void writemode(int nowmode,int nowdepth,int cptx,int cpty);
void savepicture(int depth, int line);
int copymask(int * pDepth[640*480],int masktype,int centerpointx,int centerpointy);
...
/*---Mode9-1---*/
	if(in_mode==1) //For Up and Down stairs(turn 180)
	{
		...
		while(while_juge>0) //Reference Y-axis (Stairs 4f: 750, 5f:)
		{
            ...
            if(counter_abs<=3)
            {
                if(key1!=1 && counter>=1150) //5F: 550, 4F:1150
                {
                    key1=1;
                    bodyJOGvel(600,600); //600
                    // bodyact("stop");
                ...
                if(counter<1150) //5F:550 4F:1150
                {
                    bodyact("stop");
                    usleep(5000);
                    break;
        ...
        bodyma(-175000,175000); //180 degree;-175500,175500  //170000,170000
		Check_MotorPE(1,2);
		//sleep(8);
		//bodyact("stop");
		//usleep(50000);
...
/*---Mode9-2---*/
	if(in_mode==2) //For Continuous Up stairs (4F to 5F)
	{
        ...
        printf("Step1 : First colse, Go Straight\n");
		/*---Step1 : plan A---*/
		int counter=0,counter_L=0,counter_R=0;  //declare
        int counter_abs=50;
        int while_juge=50;
        int key=0,key1=0;

		bodyJOGvel(600,600); //700 //set value
		usleep(5000);           //delay
		bodyact("fwd");         //go straight
        usleep(5000);
...
/*---Mode9-4---*/
	if(in_mode==4) //For Continuous Up stairs (3F to 4F)
	{
        printf("Mode9-4 : Continuous Up stairs tO 3F-4F\n");
        /*---Step1. First colse---*/
        printf("Step1 : First colse, Go Straight\n");
		/*---Step1 : plan A---*/
		int counter=0,counter_L=0,counter_R=0;  //declare
...
//                }
        ...
        while(while_juge>0) //Reference Y-axis (Stairs 4f: 750, 5f:)
		{
            ...
            {
                if(key1!=1 && counter>=800) //distance here
                {
                    key1=1;
                    bodyJOGvel(900,900); //600
                    // bodyact("stop");
...
/*---Mode9-5---*/
    if(in_mode==5) //For Continuous Down stairs (4F to 3F)
	{
        printf("Mode9-3 : Continuous Down stairs to 4F\n");
        /*---Step1. First colse---*/
        printf("Step1 : First colse, Go Straight\n");
...
void action_4f()
{
    ...
    printf("S1 \n");
        ...
        while(while_juge>1500) //Reference Y-axis (Stairs 4f: 750, 5f:)
		{
            ...
            for(int i = 0 ; i<frameDepth.getWidth();i++){
                counter += depthMD(i,320);
            }
            counter = (int)(counter/frameDepth.getWidth());

            printf("Aligment : counter_L = %d , counter_R = %d , counter_abs=%d \n",counter_L,counter_R,counter_abs);
            printf("counter=%d check_xtion_key=%d\n",counter,check_xtion_key);
            ...
            if(counter_abs<=3)
            {
                ...
                {
                    check_xtion_key++;
                    printf("inside/n check_xtion_key=%d",check_xtion_key);
...
void action_5f()
{
    ...
    // S5
    bodyJOGvel(1000,1000);//set value
    sleep(1);   //wait 1sec
    ...
    //printf("w_j=%d \n",while_juge);
    while(while_juge>750) //Reference Y-axis (Stairs 4f: 750, 5f:)
    {
        streamDepth.readFrame( &frameDepth );

        for(int i = 0 ; i<frameDepth.getHeight()/2;i++){
            counter_L += depthMD(340,i); //mid=320
            counter_R += depthMD(300,i); //290
        }
        counter_L = (int)(counter_L/(frameDepth.getHeight()/2));        //calculate counter average value
        counter_R = (int)(counter_R/(frameDepth.getHeight()/2));
        counter_abs=abs(counter_L-counter_R);

        for(int i = 0 ; i<frameDepth.getHeight();i++){
            counter += depthMD(320,i); //(i,300)
        }
        counter = (int)(counter/frameDepth.getWidth());
...
{
    FILE *fptr;
    fptr=fopen("modecount.txt","a");
    if(!fptr){
        printf("file error...\n");
        exit(1);
    }
    fprintf(fptr,"Nowmode=%d,Nowdepth=%d nowcenterpt=%d %d\n",nowmode,nowdepth,cptx,cpty);
    fclose(fptr);
}

int savepicture(cv::Mat mScaledDepth, IplImage* pColorImg)
{
//    char line_name[10] = "Feature";
//    char depth_name[8] = "Depth";
//
//    if (depth == 1)
//    {
//      line_name[8]=char(48+i)
//      line_name[9]='\0';
      imwrite("saveee.jpg",mScaledDepth);
//    }
//    else if (line == 1)
//    {
        cvSaveImage("savetesttt.jpg",pColorImg);
//    }
}
