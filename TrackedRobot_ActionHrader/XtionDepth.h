...
void depth_find(int dc,int *mid_x2,int *mid_y2,int *midx,int *midy,int *di,int *dj,int *dx,int *dsw)
{
    ...
    //int ptr2[row_new][col_new] = {};

/*---Dynamic Memory Allocation : new ---*/
    int **ptr2=NULL;
	ptr2= new int*[row_new];
	for(DMA_i=0; DMA_i<row_new; DMA_i++)  ptr2[DMA_i]=new int[col_new];
    for(DMA_i=0; DMA_i<row_new; DMA_i++)
    {
        for(DMA_j=0; DMA_j<col_new; DMA_j++)  ptr2[DMA_i][DMA_j]=0;
    }
    ...
    else if(dc==55)	/*---For Mode 5.5---*/
	{
		for(i=0;i<640;i++) {
			for(j=1;j<480;j++) {
				if((abs(depthMD(i,j-1)-depthMD(i,j))>4000)&&(depthMD(i,j-1)!=0)&&(depthMD(i,j)!=0))//down depth less than 4000
				{
					c=1;
					ptr2[i][j]=c;//when yes give it 1
					printf("i=%d,j=%d\t\t",i,j);
				}
			}
		}
...
/*---Dynamic Memory Allocation : delete ---*/
	...
	for(int DMA_i=0; DMA_i<row_new; DMA_i++)    delete ptr2[DMA_i];
	delete ptr2;
