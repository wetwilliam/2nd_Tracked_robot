...
int SelfMode(int *x,float *mmvalue,int imagecount,int imagecount10,int mode);
...
char window_name[12] ="image00.jpg";
char window_named[12]="depth00.txt";
char window_name2[12] ="imagg00.jpg";
char window_name3[12] ="imaga00.jpg";
...
//---------------------------------------------------------------------------------------------------------------------
  while( true )
  {
    ...
    {
       // usleep(2000000);  //inoder to let xtion picture get enough light we need to wait 1 sec
        usleep(1000000);
      // 7a. get color frame
				 ...
				 //  /*
				   ...
				   int xvve[15000],yvve[15000];
                      ...
                      for(int  i1 = 0; i1 < (lines->total); i1++)
                    {
                     ...
                     /*--*/
					  ...
					  cvCircle(pColorImg,cvPoint((int)(maxX+minX)/2,(yvvs[(lines->total)-1]+yvve[(lines->total)-1])/2),8,CV_RGB(255,255,0),-1,CV_AA,0);/////////////////////////////////////////////////////////////////////////
					   ...
					   //cvCircle(pColorImg,cvPoint((int)(maxX+minX)/2,(yvvs[1]+yvve[1])/2),8,CV_RGB(255,255,0),-1,CV_AA,0);
                      ...
                      for(int  i1 = 0; i1 < (lines->total); i1++)
                    {
					   ...
					   int maxX=xvve[count0];
					  int minX=xvvs[count0];
					   for(int  i1 = 0; i1 < lines->total; i1++) {
					  // printf(" xvvs[%d]=%d xvve[%d]=%d yvvs[%d]=%d yvve[%d]=%d ==\n",i1,xvvs[i1],i1,xvve[i1],i1,yvvs[i1],i1,yvve[i1]);
					   }
					  //
...
//////////						  }
						 ...
						 //  printf("min=%d max=%d \n",minX,maxX);
					//  cvCircle(pColorImg,cvPoint((int)(maxX+minX)/2,(yvvs[(lines->total)-1]+yvve[(lines->total)-1])/2),8,CV_RGB(255,255,0),-1,CV_AA,0);/////////////////////////////////////////////////////////////////////////
                       cvCircle(pColorImg,cvPoint((int)(maxX+minX)/2,(yvvs[count0]+yvve[count0])/2),8,CV_RGB(255,255,0),-1,CV_AA,0);
					  ...
					  window_name[6]= char(48+imagecount);
					   ...
					   centerplzx=(maxX+minX)/2;
					   centerplzy=(yvvs[count0]+yvve[count0])/2;
                         }
